...
class _ChatImageState extends State<ChatImage> {
  ...
  Future<void> downloadImage(String link, bool updateRecentGossips) async {
    String? basePath = widget.hiveHandler.tempBox.get("basepath");
    if (basePath != null) {
      String extension = widget.chat.msg.split(".")[widget.chat.msg.split(".").length-1];
      String fileStarting = widget.chat.self ? "GPIC_S" : "GPIC_R_";
      String filePath = basePath + "/Gossip_Images/$fileStarting${DateTime.now().millisecondsSinceEpoch.toString()}.$extension";
      File file = File(filePath);
      ...
      await dio.download(
        ...
        onReceiveProgress: (received, total) {
          ...
          if (total/received == 1) {
            ...
            widget.hiveHandler.gossipsBox.put(widget.myData.mid, strGossips);

            if (updateRecentGossips) {
              updateRecentGossip();
            }
